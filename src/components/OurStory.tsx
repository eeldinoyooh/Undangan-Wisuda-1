...
// Icon mapping for story items
const iconMap: Record<string, React.ReactElement> = {
  heart: <Heart className="w-5 h-5" />,
  users: <Users className="w-5 h-5" />,
  home: <Home className="w-5 h-5" />,
  rings: <Heart className="w-5 h-5" /> // Using Heart as alternative to Rings
};
...
const stories = weddingConfig.ourStory.map((story, idx) => ({
  ...story,
    ...
    <section className="py-12 px-4 bg-[#1a202c] overflow-hidden relative">
      <div className="max-w-6xl mx-auto relative z-10">
        ...
        <div className="text-center mb-12 relative" data-aos="fade-up">
          <h2 className="font-script text-4xl text-[#E6D5B8] mb-4">Our Love Story</h2>

          {/* Heart with horizontal lines */}
          <div className="flex items-center justify-center gap-2 mb-8">
            <div className="h-[1px] w-12 bg-[#E6D5B8]/40"></div>
            <div className="bg-[#1a202c] p-1.5">
              <Heart className="w-4 h-4 text-[#E6D5B8] fill-[#E6D5B8]" />
            </div>
            <div className="h-[1px] w-12 bg-[#E6D5B8]/40"></div>
          </div>
        </div>
